...
function sendOTPEmail($to, $otp) {
  ...
  $headers .= "From:" . $from;
  $subject = "Hurray!! Your OTP For Register P2pIndia is : $otp";
  ...
  <center>
    <div class="container">
      <div class="cards-container">
        <div class="card"> 
          <h1 class="success-message">Your OTP For Register P2pIndia is : ' . $otp . '</h1>
          <p>Enter Your OTP in Verify OTP to Complete The Regsiter Process</p>
...
if($success!=false){
    ...
    if ($result->num_rows === 1) {
        ...
        if ($verified === 1) {
            echo "<script>alert('Email is Alredy Registred Try Login'); window.location.href = 'login.php';</script>";
        } elseif ($tries < 5) {
          ...
          $verified = 0;
          $tries = 0;
          $insertUserQuery = "INSERT INTO users (email, otp, tries, verified, password) VALUES (?, ?, ?, ?, ?)";
          $stmt = $conn->prepare($insertUserQuery);
          $stmt->bind_param("ssssi", $email, $otp, $tries, $verified, $password);
          $stmt->execute();
          sendOTPEmail($email, $otp);
            }
} else {
